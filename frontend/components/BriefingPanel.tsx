...
export default function BriefingPanel({ briefingText, sources, loading }: BriefingPanelProps) {
  if (loading) {
    return (
  ...
  const renderBriefing = (text: string) => {
    ...
    lines.forEach((line, i) => {
      ...
      if (trimmed.startsWith('## ')) {
        flushList();
        elements.push(
          <h3 key={i} className="briefing-heading">{trimmed.replace('## ', '')}</h3>
        );
      } else if (trimmed.startsWith('- ') || trimmed.startsWith('* ') || trimmed.startsWith('• ')) {
        currentList.push(trimmed);
      } else if (trimmed.length > 0) {
        flushList();
        elements.push(<p key={i} className="briefing-paragraph">{trimmed}</p>);
      } else {
        flushList();
      }
    });
